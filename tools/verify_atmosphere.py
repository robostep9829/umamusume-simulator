#!/usr/bin/env python3
"""Checks that a biome change is *visible* in the atmosphere.
...
and if the numbers in those two files are close together - or the fog is off, or
most of the fog colour is thrown away by `fog_aerial_perspective` - then the world
keeps working and nothing changes on screen. That is the failure this checks.
...
layer, and the far layer says how far away the haze is judged.
    ...
    python3 tools/verify_atmosphere.py      # exit 0 = the transition can be seen
"""
import math
import re
import sys
...
# Thresholds of "a player would notice". The sky carries most of the screen, so it
# is the one that matters: the shift is the biome's fog colour mixed into the sky
# by `fog_sky_affect`, minus whatever `fog_aerial_perspective` hands back to the sky.
MIN_SKY_SHIFT = 0.05
...
for row in rows:
    ...
    if environment is None:
        failures.append(f"{row['name']}: no `atmosphere`, so the fog can never change for it")
        continue
    ...
    print(f"  {row['name']:10} {name:20} fog {'on ' if enabled else 'off'} "
          f"colour {tint} density {density:g}")
    print(f"             sky affect {sky_affect:g}, aerial perspective {aerial:g}, "
          f"energy {energy:g}")
    if not enabled:
        failures.append(f"{name}: `fog_enabled` is not true, so there is no fog to change")
...
fogged = [row for row in rows if row["environment"] is not None]
if len(fogged) >= 2:
    ...
    shift = max(shifts)
    print(f"strongest recolouring of the sky between two biomes: {shift:.3f}")
...
if failures:
    ...
    for failure in failures:
        print("  -", failure)
    sys.exit(1)
print("\nthe biomes' fog is on, differs where it is seen, and reaches the horizon")
